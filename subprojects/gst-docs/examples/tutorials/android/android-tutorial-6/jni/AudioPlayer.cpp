...
class VideoMessageListener : public SocketMessageListener {
public:
...
class AudioMessageListener : public SocketMessageListener {
public:
...
class VideoMessage : public ClientMessageObject {
public:
...
std::shared_ptr<XRSocketClient> g_video_client;
std::shared_ptr<XRSocketClient> g_audio_client;
std::mutex g_lock;

class VideoSocketClient : public XRSocketClient {
public:
  VideoSocketClient()
  : XRSocketClient("Video"){
...
class AudioSocketClient : public XRSocketClient {
public:
  AudioSocketClient()
      : XRSocketClient("Audio"){
    std::lock_guard<std::mutex> auto_lock(s_lock);
    s_instance_id++;
    m_name += std::to_string(s_instance_id);
  }
...
void start_sender_loop() {
  ...
  g_iteration_thread = std::make_shared<std::thread>([]() {
    while (g_sending_loop) {
      std::shared_ptr<ClientMessageObject> msg = std::make_shared<VideoMessage>();
      {
        std::lock_guard<std::mutex> auto_lock(g_lock);
        if (g_video_client) {
